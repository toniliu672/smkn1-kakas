...
{
    try {
        // Generate ID yang lebih random dengan timestamp dan 6 digit random
        $timestamp = date('ymd');
        ...
        // Validasi NIS dan NISN jika ada
        if (!empty($data['nis'])) {
        ...
        // Insert data siswa
        $query = "INSERT INTO siswa (id, id_angkatan, id_jurusan, nis, nisn, nama_lengkap, 
                 foto_siswa, alamat, no_hp, email, nama_ayah, nama_ibu, 
        ...
        $pdo->commit();
        return ['status' => 'success', 'message' => 'Data siswa berhasil ditambahkan', 'id' => $id];
    } catch (Exception $e) {
        $pdo->rollBack();
